...
class MNIST_Dataset(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        mean = dat.mean()
        std = dat.std()
        #Normalization idea from : https://deeplizard.com/learn/video/lu7TCu7HeYc 
        transform = transforms.Normalize(mean,std)
        dat = transform(dat)
        return dat, self.labels[index]

'''function to compute accuracy between model probability predictions and expected class value inspired by Dr. Agnieszka Ławrynowicz class materials: https://colab.research.google.com/drive/1mcB0tftQihL3iTKSBxtGOEzuc_34VQGa?usp=sharing '''
...
def train(train_data,valid_data, model, cost, opt, n_epoch = 100, batch_size = 64):
    ...
    #this approach was inspired by : https://pythonguides.com/pytorch-early-stopping/ 
    prev_loss = 10000 #start out of range
    patience = 3 #num epochs to wait 
    triggers = 0 #count depreciation occurances 

    # this iterative approach to training drew from Dr. Agnieszka Ławrynowicz class materials: https://colab.research.google.com/drive/1mcB0tftQihL3iTKSBxtGOEzuc_34VQGa?usp=sharing
    for epoch in range(n_epoch):
        ...
        print(epoch, ":", torch.tensor(epoch_loss).mean())
